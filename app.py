...
st.markdown(f"""
    ...
    .cell-value {{ position: relative; z-index: 2; font-weight: bold; color: #333; }}
    ...
    try:
        ...
        if not filtered.empty:
            ...
            for val, label, color, dash in stats_config:
                ...
                fig.add_trace(go.Scatter(x=[None], y=[None], mode='lines', line=dict(color=color, width=3, dash=dash), name=f"{label}: {int(val)}s", showlegend=True), secondary_y=False)
